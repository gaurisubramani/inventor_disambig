...
/**
  * Load mentions from a mongo db based on inventor ids
  */
trait LoadMentionsFromMongo {

  val datastore: Datastore[String, InventorMention]

  def load(ids: Iterable[String]) = {
    println(s"""Loading mentions with ids: ${ids.mkString(",")}""")
    ids.map(datastore.get(_).head)
  }
...
abstract class StandardParallelCoreference(override val allWork: Iterable[CorefTask],
  ...
  override def finalizeOutput(): Unit = writer.collectResults(

    outputDir, new File(outputDir, "all-results.txt"))

  override def getMentions(corefTask: CorefTask): CorefTaskWithMentions = {
    new CorefTaskWithMentions(corefTask.name, corefTask.ids, load(corefTask.ids))
  }
}
...
class TextCorefOutputWriter(outputDirectory: File, codec: String = "UTF-8", convertToOneBased: Boolean = true) extends CorefOutputWriter[BasicCorefOutputRecord] {
  ...
  override def collectResults(outputDir: File, collectedFile: File): Unit = {
    ...
    results.foreach(f => {
      pwToScore.println(s"${f.mentionId}\t${f.disambiguatedId}");
      pwToScore.flush()
    })
    pwToScore.close()
  }
